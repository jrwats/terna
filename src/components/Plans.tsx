      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {(["semaglutide", "tirzepatide"] as Medication[]).map((m) => (
              <button
                ...
                className={`px-6 py-2.5 rounded-full text-sm font-semibold capitalize transition-all ${
                  med === m
                    ? "bg-brand-light text-gray-900 shadow-lg"
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {plans.map((plan, i) => {
            ...
            return (
              <Reveal key={plan.months} delay={i * 100} className="flex flex-col">
                <div
                ...
                >
                  {plan.popular && (
                    <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1 bg-brand-light text-gray-900 text-xs font-bold rounded-full shadow">
                      Most Popular
                    </span>
                  )}
                  ...
                  <ul className="space-y-3 mb-8 flex-1">
                    {features.map((f) => (
                      <li key={f} className="flex items-start gap-3 text-sm">
                        <Check
                          size={16}
                          className="mt-0.5 flex-shrink-0 text-brand-light"
                        />
                        <span className={plan.popular ? "text-white/85" : "text-gray-300"}>
                          {f}
                        </span>
                      </li>
                    ))}
                  ...
                  <a
                    ...
                    className={`block text-center py-3 rounded-full font-bold text-sm transition-colors ${
                      plan.popular
                        ? "bg-brand-light text-gray-900 hover:bg-brand-mid"
